...
- **Async HTTP wrapper:** `backend/ejudge/client.py` exposes `EjudgeClient`, an async helper that translates the models into correctly encoded HTTP requests (form, multipart, or query string) and validates responses. It normalises authentication headers (`Authorization: Bearer AQAA<TOKEN>`), targets the `/ej/api/v1/{client,master}` REST endpoints directly, and raises rich exceptions (`EjudgeClientError`, `EjudgeReplyError`) for both transport failures and `ok=false` replies. Every operation described in `backend/ejudge/ejudge_doc.json` is also exposed as a convenience coroutine under `EjudgeClient.client` or `EjudgeClient.master`.
- **REPL bridge:** `backend/ejudge/repl.py` wraps the async client inside a synchronous `SyncEjudgeClient`, powers the `make repl` target, and pre-wires `code.interact` with useful helpers so engineers can probe ejudge endpoints from a plain Python shell.
- **Statement renderer:** `backend/ejudge/statements.py` emits ejudge-compatible `statement.xml` files by wrapping arbitrary HTML fragments inside CDATA sections and ensuring they survive config reloads even when the source contains mismatched tags.
